...
| 数据库（PG17+pgvector） | `docker start workloom-im-pg` | `docker inspect` health=healthy |
| 数据库迁移 | `pnpm db:migrate` | 迁移版本号推进无报错 |
| 启动双端 | `pnpm dev` → server:8787（tRPC `/trpc/*`）+ web:5173 | `curl -o /dev/null -w %{http_code} localhost:5173` = 200（8787 无 /healthz 属正常） |
| 酒店获客演示种子 | `pnpm db:seed` | 种子回报事件/员工/对象计数 |
| 短视频域种子 | `pnpm db:seed:video` | 同上 |
| GEO 域种子（60 事件） | `pnpm db:seed:geo` | 同上 |
...
| GEO 域套件（77 条） | `pnpm suite:geo` | 全绿 |
| 酒店域套件（43 条） | `pnpm suite:hotel` | 全绿 |
| **发布门禁（未全过禁止发布）** | `pnpm release:gate` | 全项通过，exit 1=禁止发布 |
| 五元事件验链 | `pnpm db:verify-chain` | 链完整无断点 |
| 类型检查 / 单元测试 | `pnpm typecheck` / `pnpm test` | 全绿 |

> 注意：跑测试前**先停掉 8787/5173 残留服务**——残留服务不仅致 E2E 打错库，dev 侧夜班/扩编等后台节拍产生的提案与事件还会污染套件断言（实测 R-26 误报）。若库已被运行态数据污染：重建库 → `pnpm db:migrate && pnpm db:seed` 后重跑。

## L3 系统内自动化层 · packages/base 底座能力

这些是系统**自身**内置的自动化机制，改代码时优先复用，不要绕过：

| 包 | 能力 | 关键点 |
|---|---|---|
| `publish-rpa` | 全平台 RPA 发布（抖音/小红书/B站/YouTube 参考实现 + TikTok/视频号占位） | 适配器不 import playwright，注入 `BrowserDriver` 接口即可接真浏览器（接法见 agent-computer-guide §4） |
| `fence-engine` | 围栏 DSL 执行引擎（事前裁决） | 支持列表字面量/`in`/`contains`/`contains_any`；缺失路径在比较语境宽容 |
...
| `asset-cms` / `cost-ledger` / `deal-flow` / `social-listening` | 资产管理 / 成本台账 / 交易流 / 社媒监听 | 见各自 README |
...
系统自带的可复用技能（Agent 可按 SKILL.md 加载执行）：

`release-gate`（发布门禁 SOP）· `industry-entry`（行业接入向导）· `product-feedback`（产品反馈闭环）· `demo-mirror`（演示镜像）· `cross-platform-review`（跨端走查）· `deal-flow`（交易流）· `ripping-reverse`（拆解逆向）

## L6 演示资产层 · docs/demo + 设计规范

| 资产 | 位置 | 用途 |
|---|---|---|
| 高保真演示页（8 页，糖果色） | `docs/demo/`（含 hotel-guest-app / hotel-owner-app 等） | 直接用浏览器打开演示三端 |
...
1. **computer-use 浏览器操作**——不跑 preflight 直接 `browser_connect` 必失败，导致"系统跑起来了但从没被真正打开看过"。
2. **`pnpm doctor`**——环境问题排查入口，比盲目重装快得多。
3. **`pnpm release:gate`**——发布门禁是硬性纪律，不是可选项。
4. **`pnpm db:verify-chain`**——改事件/号源相关代码后必跑。
5. **publish-rpa 的 BrowserDriver 注入点**——系统真机发布能力的接缝，接上 CDP 即可真跑。
6. **skills/official/**——仓库自带技能，别在外面临时发明流程。
7. **docs/demo 演示页**——向人展示系统时直接用，不用现做 PPT。
