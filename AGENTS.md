...
> 你正在 WorkLoom（AI 获客系统）仓库中工作。本文件是你进入仓库后**最先要读**的文件。
...
pnpm db:seed                         # 酒店获客演示种子（可选；geo/video 域另有 db:seed:geo / db:seed:video）
```
...
- 改完代码必须跑：`pnpm suite`（445 条）；涉及 GEO/酒店域再跑 `pnpm suite:geo` / `pnpm suite:hotel`
