...
| 层 | 来源 | 说明 |
|---|---|---|
| 业务数据 | `scripts/seed*.ts`（`pnpm db:seed` / `db:seed:video` / `db:seed:geo`，幂等） | 云栖酒店获客演示数据集：员工/围栏/对象/事件/订单/线索/券/会话/工单/知识库 385 问 |
...
pnpm db:migrate && pnpm db:seed   # 需要 video/geo 域再补 db:seed:video / db:seed:geo
```
