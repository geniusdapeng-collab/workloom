...
获客版是模型路由价值的最大秀场——**三个行业 bundle 共享同一底座路由内核，每一次模型调用按场景定档、按倍率计量、按事件可审，获客链路的每一分钱都花得清楚。**
...
- **vendor 引擎全量收口**：SuperMickey 预生产 14 个 LLM agent 全部经 routeSmart 路由（真实计量 + 降级链留痕，vendor 只读不破）
- **渲染额度台账 + 预算闸**：套餐秒数配额（标准 100s / 智能 500s），G8 围栏后超支未确认即熔断；Seedance → 可灵 → 即梦降级链 + 轮询回填
- **积分三池账本 + 加油包**：P1「💎 积分账本」面板，五档加油包 450→13,800 元；售前体检 `bill_to: platform` 平台成本——获客补贴烧在刀刃上
- **👍/👎 反馈环**：Ask 应答卡一键升级重答（点踩换更强大脑，首次免费），反馈数据回流驱动路由调表（升级率 >15% 场景进 CEO 晨报风险栏）
